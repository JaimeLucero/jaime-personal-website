...
const LABEL_CLASS_NAME = 'type-label block text-panel-ink/60';
    ...
    <div className="grid gap-12 lg:grid-cols-5">
      <form data-testid="contact-form" onSubmit={handleSubmit} className="space-y-7 lg:col-span-3">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <button
            type="submit"
            className="inline-flex shrink-0 items-center justify-center whitespace-nowrap bg-accent px-6 py-3 font-medium text-paper transition-colors hover:bg-accent-strong">
            Send Project Brief
          </button>
          <span className="type-small text-panel-ink/60">Opens your email app with the message filled in.</span>
        </div>
      </form>

      <div className="lg:col-span-2">
        <p className="type-eyebrow">Or reach me directly</p>
          ...
          {PROFILE_LINKS.map((profileLink) => (
            <li key={profileLink.label}>
              <a
                ...
                <span className="type-item-title text-panel-ink">{profileLink.label}</span>
                <span className="type-label text-right normal-case tracking-normal text-panel-ink/60 [overflow-wrap:anywhere] group-hover:text-panel-ink">{profileLink.handle}</span>
              </a>
            </li>
          ))}
        </ul>
